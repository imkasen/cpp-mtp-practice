...
int main(int argc, char *argv[])
{
	...
	if (argc < 2) { fprintf(stderr, "usage: %s fractal_size\n", argv[0]); exit(-1); }
	size = atoi(argv[1]);
	...
	char buffer[20];
	snprintf(buffer, 20, "fracout_%d.tga", size);
	
	//allocate memory for the fractal
	unsigned int* pixmap = (unsigned int*)malloc(size*size*sizeof(int));

	for(int i = 0; i < ITERATIONS; i ++) {
		if(i < ITERATIONS && i > 0) //reallocate memory for each iteration
			pixmap = (unsigned int*)malloc(size*size*sizeof(int));	

		//start the timer
		times[i]= mysecond();	
		
		//run the mandelbrot function
		mandelbrot((float)size, (float)size, pixmap);
	...
	writetga(pixmap, size, size, buffer);

	//free the memory
	free(pixmap);
    
	//print the results (see lib.cpp)
    print_results(times);
