...
void read_matrix(const std::string &input_filename,
    ...
    while (std::getline(infile, line)) {
        ...
        while (std::getline(line_stream, token, ' ')) {
            matrix.at(count) = std::stof(token);
            count++;
        }
        line_stream.clear();
    }
...
void calc_mean(const std::vector<float> &matrix,
    ...
    for (int i = 0; i < rows; ++i) {
        ...
        for (int j = 0; j < cols; ++j) {
            sum += matrix.at(i * cols + j);
        }
        mean.at(i) = sum / static_cast<float>(cols);
    }
}
...
void calc_mm_std(const std::vector<float> &matrix,
    ...
    for (int i = 0; i < rows; ++i) {
        float sum = 0.0f;
        for (int j = 0; j < cols; ++j) {
            float diff = matrix.at(i * cols + j) - mean.at(i);
            minus_mean.at(i * cols + j) = diff;
            sum += diff * diff;
        }
        std_dev.at(i) = std::sqrt(sum);
    }
}
...
void pearson(const std::vector<float> &minus_mean,
    ...
    for (int sample1 = 0; sample1 < rows - 1; ++sample1) {
        ...
        for (int sample2 = sample1 + 1; sample2 < rows; ++sample2) {
            float sum2 = 0.0f;
            for (int j = 0; j < cols; ++j) {
                sum2 += minus_mean.at(sample1 * cols + j) * minus_mean.at(sample2 * cols + j);
            }
            float r = sum2 / (std_dev.at(sample1) * std_dev.at(sample2));
            output.at(sample1 * rows + sample2 - sum1) = r;
        }
    }
}
